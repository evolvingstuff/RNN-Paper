...
def concat_and_multiply(weights, add_bias, *args):
    if add_bias:
...
def create_rnn_params(input_size, state_size, output_size,
    ...
    return {'init hiddens': rs.randn(1, state_size) * param_scale,
            'IH':   	    rs.randn(input_size+1, state_size) * param_scale,
            'HH':    		np.ones((state_size, 1)) * param_scale,
            'HO':           rs.randn(state_size+1, output_size) * param_scale}

def rnn_predict(params, inputs):
    def update_rnn(input, hiddens):
        #return np.tanh(concat_and_multiply(params['change'], input, hiddens))
        ih = concat_and_multiply(params['IH'], True, input)
        hh = concat_and_multiply(params['HH'], False, hiddens)
        tot = ih + hh
        return np.abs(tot)

    def hiddens_to_output_probs(hiddens):
        output = concat_and_multiply(params['HO'], True, hiddens)
...
if __name__ == '__main__':
    ...
    def callback(weights, iter, gradient):
    	#for i in xrange(len(weights['HH'])):
    	#	weights['HH'][i] = 1.0
        if iter % 10 == 0:
