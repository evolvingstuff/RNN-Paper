...
%footnote?
Somewhat unexpectedly, it turns out that some tasks can be solved even if the recurrent weights are held constant at 1, or even randomly selected values in the range $\left [-1, 1 \right ]$. However, it is more often the case that allowing the weights to be tuned by learning leads to better generalizing models.
...
Real-time recurrent learning (RTRL) is a online approach to training RNNs \citep{RTRL}. Instead of building a deep computational graph extending back into the past, all partial derivatives are updated and carried forward with each timestep. RTRL is not currently in use due to computational inefficiency as compared to back-propagation through time (BPTT). Consequently, modern RNNs are generally trained using full or truncated versions of BPTT. 
...
RTRL for fully connected RNNs requires the storage of $n^3 + mn^2$ partial derivatives, whereas for Diagnet that requirement is reduced to $n (m + 1)$.
...
%TODO: should we call the "experiments" plural? Maybe should consider adding another task of some kind?
