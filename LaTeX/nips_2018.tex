...
\begin{figure}[h]
  \centering
  \includestandalone[scale=1.0]{figures/diagnet1}
  \caption{Diagnet structural diagram.}
\end{figure}

%\begin{equation}
%  \mathbf{h}_t := \textrm{abs}( \mathbf{u} \circ \mathbf{h}_{t-1} + \mathbf{Vx}_t )
%\end{equation}

%Note that abs() is a vectorized function, applied element-wise to each compontent of its argument.
...
One significant danger in the use of the absolute value activation function is that, without special care and caution, the magnitude of the hidden state can violently explode. Fortunately this problem is avoidable by constraining the magnitude of recurrent weights the range [-1, 1]. This can be interpreted as a very simplistic way of limiting spectral radius.
