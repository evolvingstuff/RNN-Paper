...
An absolute value activation function in the recurrent hidden layer has been chosen due to some of its desirable properties:
...
\begin{itemize}
%\item \textbf{Non-saturating}: Completely prevents loss of sensitivity to small changes of input which can occur, for example, in sigmoidal functions, or in ReLU for negative inputs.
%TODO: this analogy is fuzzy and needs to be tightened, because it doesn't mention the need for absence of new input
\item \textbf{Idempotent}: Assuming an absence of new input, and with a self-recurrent connection of 1.0, long term, high precision storage of information is possible without any specialized architectural design.
...
\item \textbf{Backward pass norm-preserving}: Similarly, corrective gradient vectors being passed back through the computational graph enjoy the same benefits as the activation vectors in the forward direction.
\end{itemize}
