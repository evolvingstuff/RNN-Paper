...
%\sqrt{{\frac{\partial f}{\partial x_k}}^{2}} = 
%\sqrt{{ \left | {\frac{\partial f}{\partial x_k}} \right | }^{2}} = 
%\left | {\frac{\partial f}{\partial x_k}} \right |
%\end{equation}
%\begin{equation}
%{\left \| \Delta f \right \|}_2 = 
%\sqrt{{\frac{\partial f}{\partial x_1}}^{2} + \dots + {\frac{\partial f}{\partial x_k}}^{2}} = 
%\sqrt{{\left | {\frac{\partial f}{\partial x_1}} \right | }^{2} + \dots + {\left | {\frac{\partial f}{\partial x_k}}\right | }^{2}} = 
%{\left \| \textrm{abs}( \Delta f ) \right \|}_2
%\end{equation}
...
One significant danger in the use of the absolute value activation function is that, without special care and caution, the magnitude of the hidden state can violently explode. If the recurrent matrix has a spectral value greater than 1, a cycle of unbounded self amplification can quickly lead to numeric overflow. Fortunately this problem is avoidable by constraining the magnitude of recurrent weights to the range [-1, 1]. This can be interpreted as a very simplistic way of limiting spectral radius.

%footnote?
Somewhat unexpectedly, it turns out that some tasks can be solved even if the recurrent weights are set to fixed values of 1. However, it is more often the case that allowing the weights to change with learning leads to better models.
...
\subsection{Efficient Real-Time Recurrent Learning Implementation}

Real-time recurrent learning (RTRL) is a alternative approach to training RNNs \citep{RTRL}. It is not currently in use due to computational inefficiency as compared to backpropagation through time (BPTT). Modern RNNs are generally trained using BPTT. However, Diagnet's decoupled hidden layer eliminates any exchange of information between the hidden neurons, and as a consequence allows for a significantly more efficient RTRL implementation.
...
\begin{equation}
{\left[ {\frac{\partial f_j}{\partial w_j}} \right]}_t := \textrm{sgn} \left( \left[ \sum_{i, j} x_{i_t} w_{ij }\right] + r f_{j_{t-1}} \right) x_{j_t} + r {\left[ {\frac{\partial f_j}{\partial w_j}} \right]}_{t-1}
\end{equation}

Big O analysis of this.
