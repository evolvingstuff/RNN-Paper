...
Real-time recurrent learning (RTRL) is a alternative approach to training RNNs \citep{RTRL}. Instead of building a deep computational graph extending back in time, all partial derivatives are maintained and carried forward with each timestep. It is not currently in use due to computational inefficiency as compared to backpropagation through time (BPTT). Modern RNNs are generally trained using BPTT. However, Diagnet's decoupled hidden layer eliminates any exchange of information between the hidden neurons, and a fortunate side-effect of that is that there exists a significantly more efficient RTRL implementation.
