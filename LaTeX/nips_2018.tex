...
Recurrent neural networks are a challenge to train, in large part due to the well known problem of vanishing and exploding gradients. They are also much more computationally expensive to evaluate and train than their feedforward counterparts. Attempts to circumvent these problems have led to increasingly complex architectures and training methods. We propose Diagnet, a simple RNN design which consists of three core components: 1) an efficient diagonal hidden-to-hidden recurrent matrix, 2) the absolute value function as the element-wise nonlinearity, and 3) the application of constraints to both the recurrent parameter weights and gradient norms during training. Despite the segregated and simplistic nature of Diagnet's recurrent units, significant generalization performance improvement is seen over Long Short-Term Memory on the 1k version of the bAbI tasks. Diagnet scores higher in testing on 19 out of the 20 tasks, by margins of up to 40\% accuracy.
...
The absolute value function was chosen as the recurrent nonlinearity because it displays a number of desirable properties: 
