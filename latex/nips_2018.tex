...
To recap the core elements of Diagnet’s architecture: 1) A single layer of segregated recurrent units which are self-connected 2) The absolute value function (an unbounded norm-preserving idempotent nonlinearity), and 3) Application of constraints in both recurrent parameter weights and gradient norms, which tightly control the exploding gradient problem.  Despite the segregated and simplistic nature of Diagnet's recurrent units, significant performance improvement is seen in the 1000-example version of the bAbI task over LSTM and GRU. In addition, Diagnet scores higher in testing accuracy on 15 out of bAbI 20  bAbI tasks, by margins of 3-30%, while supporting more hidden layers than many other RNN architectures. 
It’s super duper fast. Hadamard product FTW!!


Speculate that Diagnet may turn out to be a good component to be used in more complex hybrid neural solutions, much like LSTM and GRU are used currently.

Future directions? Add data augmentation, attentional mechanisms, small-worlds connectivity between nodes in hidden layer, better characterize the theoretical limitations of this architecture in terms of Turing completeness, grammars, etc..

Limitations? Currently appears to only handle problems in which it is largely input driven. It will not do as well, I think, in situations where it is expected to produce sequences of outputs after arbitrary delays.
(Suspect it could not handle a variation of pathological adding wherein you subtract the second flagged input rather than adding it. However, I’m not sure if LSTM or GRU can do this over long delays either…)

Link to source code somewhere
...
%Fahlman, S.E. (1990). The Recurrent Cascade-Correlation Architecture. NIPS.
%
